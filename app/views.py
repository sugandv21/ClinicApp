...
def dashboard():
    if current_user.is_doctor:
        ...
        my = Appointment.query.filter_by(patient_id=current_user.id).order_by(Appointment.date, Appointment.time).all()
...
def doctor_schedule():
    ...
    if request.method == 'POST':
        ...
        av = DoctorAvailability(doctor_id=current_user.id, date=d, start_time=start, end_time=end, slot_minutes=slot)
        db.session.add(av)
        db.session.commit()
        flash('Availability saved.', 'success')
        return redirect(url_for('main.doctor_schedule'))
    avail = DoctorAvailability.query.filter_by(doctor_id=current_user.id).order_by(DoctorAvailability.date.desc()).all()
...
def book_slot(doctor_id):
    ...
    # Collect all availabilities for that doctor & date
    ...
    for av in avails:
        ...
        booked = {a.time for a in Appointment.query.filter_by(doctor_id=doctor.id, date=selected_date).all()}
        for t in slots:
            if t not in booked:
    ...
    if request.method == 'POST':
        ...
        exists = Appointment.query.filter_by(doctor_id=doctor.id, date=selected_date, time=chosen_time).first()
        if exists:
            flash('Sorry, that slot just got booked. Pick another.', 'danger')
            return redirect(url_for('main.book_slot', doctor_id=doctor.id, date=selected_date.isoformat()))
        appt = Appointment(doctor_id=doctor.id, patient_id=current_user.id, date=selected_date, time=chosen_time, status='confirmed')
        db.session.add(appt)
        db.session.commit()
...
def send_reminders():
    ...
    q = Appointment.query.filter(
        ...
        Appointment.reminder_sent == False,  # noqa
    ).all()
    ...
    for appt in q:
        ...
        if now <= appt_dt_utc <= soon:
            ...
            send_email(
                ...
                body=f"Reminder: Appointment with Dr. {appt.doctor.full_name} on "
                     f"{appt.date.strftime('%d-%m-%Y')} at {appt.time.strftime('%H:%M')}."
            )
            send_email(
                subject="Upcoming Appointment Reminder",
                recipients=[appt.doctor.email],
                body=f"Reminder: Appointment with {appt.patient.full_name} on "
                     f"{appt.date.strftime('%d-%m-%Y')} at {appt.time.strftime('%H:%M')}."
            )
